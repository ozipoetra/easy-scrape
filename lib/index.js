...
export function easyScrape(input, schema, options = {}) {
  ...
  try {
    ...
    const fallbackHtml = input.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '')
                      .replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '');
    try {
      $ = cheerio.load(fallbackHtml, parseOptions);
    } catch (fallbackError) {
      throw new Error(`Failed to parse HTML: ${error.message}`);
    }
  }

  const debugLog = options.debug ? (...args) => console.log('[easy-scrape]', ...args) : () => {};
  const selectorPath = [];

  const normalizeOpt = (opt) => {
    if (typeof opt === "string") opt = { selector: opt };

    // Handle $ref - schema reference
    if (opt.$ref && refSchemas[opt.$ref]) {
      const refSchema = refSchemas[opt.$ref];
      opt = { ...refSchema, ...opt };
    }
  ...
  const handleDataObj = (data, context = $) => {
    ...
    for (const [name, rawOpt] of Object.entries(data)) {
      ...
      // Track selector path for debug/errors
      selectorPath.push(name);
      const currentPath = selectorPath.join('.');

      debugLog(`Processing: ${currentPath} (selector: "${opt.selector || '(context)'}")`);
...
function extractText($el, opt) {
  const whitespace = opt.whitespace || 'normal';
  
  if (opt.textMode === 'ownText') {
    let text = $el.contents()
      .filter((i, el) => el.type === 'text')
      .text();
    return whitespace === 'collapse' ? text.replace(/\s+/g, ' ').trim() :
           whitespace === 'preserve' ? text : text.trim();
  ...
  return whitespace === 'collapse' ? text.replace(/\s+/g, ' ').trim() :
         whitespace === 'preserve' ? $el.text() : text;
}
...
function extractValue($el, opt, $) {
  ...
  if (typeof value === "string" && opt.trimValue && opt.whitespace !== 'preserve') {
    value = value.trim();
  }
  ...
  // Handle whitespace mode for regular text extraction
  if (opt.whitespace && typeof value === 'string') {
    if (opt.whitespace === 'collapse') {
      value = value.replace(/\s+/g, ' ').trim();
    } else if (opt.whitespace === 'preserve') {
      // keep as-is, already skipped trim above
    } else if (opt.whitespace === 'normal') {
      value = value.trim();
    }
  }
 ...
 * easyScrape(html, '.title') → 'Hello World'
 */
export function pluck(input, selector, options = {}) {
  const result = easyScrape(input, { _tmp: selector }, options);
  return result._tmp;
}
 ...
 */
export function extractAll(input, selector, options = {}) {
  if (!input) {
    throw new Error('Input cannot be null or undefined');
  }
  ...
  return elements.get().map(el => $(el));
}

/**
 * Create schema reference for reuse
 */
export function createRef(name, schema) {
  return { $ref: name };
}

let refSchemas = {};
export function registerRef(name, schema) {
  refSchemas[name] = schema;
}
